#include "perf.h"

#include "memory.h"
#include "fiber.h"
#include "tuple.h"

#include <math.h>

/**
 * This function creates new tuple with refs == 1.
 */
static inline struct tuple *
create_tuple(const char *start, const char *end)
{
	struct tuple *ret = tuple_new(box_tuple_format_default(), start, end);
	tuple_ref(ret);
	return ret;
}

static inline void
test_unref(struct tuple **array, int size)
{
	for (int i = 0; i < size; i++)
		tuple_unref(array[i]);
}

static inline void
test_tuple_access(struct tuple **array, int size)
{
	uint64_t sum = 0,
		 counter = 0;
	uint32_t out;
	struct cur_time start = perf_get_time(NULL);
	for (int j = 0; j < size; j++) {
		struct tuple *tuple = array[j];
		sum += tuple->is_dirty;
		sum += tuple_data_or_null(tuple) == NULL;
		for (uint32_t k = 0; k < tuple_field_count(tuple) &&
				     k < 10000; k++) {
			counter++;
			sum += tuple_field_u32(tuple, k, &out);
		}
	}
	struct perf_time time = perf_count(start);
	assert(sum == 0);
	printf("%s\n", perf_json_result("Tuple access in different ways",
	                                "access / s",
					counter / time.wall_time));
	printf("%s\n", perf_json_result("Tuple access in different ways",
	                                "access / s",
					counter / time.cpu_time));
}

struct alloc_perf {
	struct perf_time time;
	uint64_t overall_size;
};

static inline void
test_alloc(struct tuple **array, char *prealloc, size_t data_size,
	   float factor, int amount, struct alloc_perf *perf)
{
	struct perf_time time = perf_init();
	uint32_t overall_size;
	for (int i = 0; i < amount; i++) {
		char *end = prealloc;
		end = mp_encode_array(end, data_size);
		for (size_t j = 0; j < data_size; j++)
			end = mp_encode_uint(end, j);
		struct cur_time start = perf_get_time(NULL);
		array[i] = create_tuple(prealloc, end);
		perf_add_time(&time, perf_count(start));
		overall_size += array[i]->bsize;
		data_size = ceil(factor * data_size);
	}
	perf->overall_size += overall_size;
	perf_add_time(&perf->time, time);
}

int
main(void)
{
	memory_init();
	fiber_init(fiber_c_invoke);
	tuple_init(NULL);

	size_t prealloc_size = 100 * 1024 * 1024;
	char *prealloc = malloc(prealloc_size);

	size_t data_size = 2;
	float factor = 1.4;
	int amount = log(prealloc_size / (data_size * sizeof(uint))) /
		     log(factor);

	struct alloc_perf alloc_perf;
	alloc_perf.time = perf_init();
	alloc_perf.overall_size = 0;
	int repeat_alloc = 20;
	struct tuple **array = calloc(amount, sizeof(struct tuple *));
	for (int i = 0; i < repeat_alloc; i++) {
		test_alloc(array, prealloc, data_size,
			   factor, amount, &alloc_perf);
		if (i == repeat_alloc - 1) break;
		test_unref(array, amount);
	}
	printf("%s\n%s\n",
	       perf_json_result(tt_sprintf("Tuple allocation,"
					   "bsize from %u to %u with factor %f",
					   array[0]->bsize,
					   array[amount - 1]->bsize,
					   factor),
				"bytes / s", alloc_perf.overall_size /
					     alloc_perf.time.wall_time),
	       perf_json_result(tt_sprintf("Tuple allocation,"
					   "bsize from %u to %u with factor %f",
	                                   array[0]->bsize,
					   array[amount - 1]->bsize,
					   factor),
				"bytes / s", alloc_perf.overall_size /
					     alloc_perf.time.cpu_time));

	test_tuple_access(array, amount);

	test_unref(array, amount);

	free(array);
	free(prealloc);

	tuple_free();
	fiber_free();
	memory_free();
}
